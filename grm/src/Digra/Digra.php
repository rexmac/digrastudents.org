  ...
  */
  ...
  public function fetchResearchPositions() {
    ...
    */
    ...
    $response = file_get_contents('./grm.html');
    #$filename = $cacheDir . '/positions.raw.html';
    $filename = './grm.raw.html';
  ...
  public function parseResearchPositionsHtmlToCsv($html = false, $csvOut = null) {
    if(!$html) {
    ...
    // Map column headers to array keys
    $headers = array(
      'continent'  => 'Continent',
      'country'    => 'Country',
    ...
    $filename = $csvOut === null ? './grm.csv' : $csvOut;
    if(!($fh = fopen($filename, 'w'))) {
      throw new \RuntimeException('Failed to open output file for writing CSV data.');
    }
    fputcsv($fh, $headers);
    foreach($data as $fields) {
      fputcsv($fh, $fields);
  ...
  public function csvToJson($csv, $json) {
    ...
    $filename = $json === null ? './grm.json' : $json;
